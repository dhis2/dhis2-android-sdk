...
public class DeletedObjectEndPointCall implements Call<Response<Payload<DeletedObject>>> {
    ...
    public Response<Payload<DeletedObject>> call() throws Exception {
        ...
        if(lastSyncedDeletedObjects == null) {
            deletedObjectsByLastUpdated = deletedObjectService.getDeletedObjects(getAllFields(),
                    true,
                    deletedObjectKlass).execute();
        }else{
            deletedObjectsByLastUpdated = deletedObjectService.getDeletedObjectsDeletedAt(getAllFields(),
                    true,
                    deletedObjectKlass, lastSyncedDeletedObjects).execute();

        }
        if (deletedObjectsByLastUpdated.isSuccessful() && deletedObjectsByLastUpdated.body().items() != null) {
