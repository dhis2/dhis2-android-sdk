...
- [KSP](https://github.com/google/ksp) - Kotlin Symbol Processing for code generation (model builders, DAO queries, migrations)
...
- `*DIModule`: Koin module (annotated with `@Module` and `@ComponentScan`) that exposes the injectable classes of the feature package, like store, handler and service. Dependencies are declared with `@Singleton` on the implementation classes and the bindings are generated at compile time by KSP (Koin Annotations).
